...
class Cluster:
    """Maintains the state of a cluster"""
    ...
    def set_cluster_timeseries(self,list_ts):
        self.list_of_timeseries = OrderedDict(sorted(list_ts.items(), \
            key=lambda t: t[0]))
        cluster_size = len(self.list_of_timeseries)
        self.statistics = Statistics(cluster_size)
        self.update_statistics(init=True) # only calculates matrices
    ...
    def calcula_rnomc_dict(self,init=False):
        ...
        for k in self.statistics.rnomc_dict:
            self.statistics.rnomc_dict[k] = \
                sqrt( (1-self.statistics.corr_dict[k]) / 2 )
    ...
    def calcula_distances_coefficients(self):
        """Calc dist pars needed in 3.4.1 and 3.4.3 of the paper"""
    ...
    def calcula_hoeffding_bound(self,init=False):
        """Calc hoeffding bound (epsilon) as proposed in 3.4.1"""
        r_sqrd = 1  # because the data is normalized
        self.statistics.hoeffding_bound = \
            sqrt(r_sqrd * log(1/self.confidence_level) \
    ...
    def get_smaller_distance_with_pivot(self, pivot_1, pivot_2, current):
        """Look for the distance in rnomc_dict

        By using 2 pivots and an index
        """
        ...
        dist_1 = self.statistics.rnomc_dict[(min(pivot_1,current), \
            max(pivot_1,current))]
        dist_2 = self.statistics.rnomc_dict[(min(pivot_2,current), \
            max(pivot_2,current))]
    ...
    def split_this_cluster(self, pivot_1, pivot_2):
        ...
        ### After creating 2 lists based on 2 pivots, it's time
    ...
    def test_split(self):
        """Test if splitting is necessary and perform split"""
        if (self.statistics.n_of_instances >= self.n_min
            and self.statistics.dist_dict_coef.get('d2_val') is not None):
    ...
    def test_aggregate(self):
        """Test if aggregating is necessary and perform aggregation"""
        if (self.parent is not None
            and self.statistics.n_of_instances >= self.n_min
            and self.statistics.dist_dict_coef.get('d1_val') is not None):

            if (self.statistics.dist_dict_coef['d1_val']
                - self.parent.statistics.dist_dict_coef['d1_val']
                > max(self.statistics.hoeffding_bound,
...
class Node_of_tree(Cluster, NodeMixin):
    """Extension to class Cluster to use tree structure"""
    ...
    def print(self):
        for pre, fill, node in RenderTree(self):
            print("%s%s %s %s" % ( \
                ...
                node.list_timeseries_names() if node.active_cluster \
                else " [NOT ACTIVE]" ))
